...
document.getElementById("get-started-btn").addEventListener("click", () => {
  ...
  document.getElementById(
    "modal-user"
  ).innerText = `${nam},  চলুন আজ নতুন কিছু শেখা যাক`;
...
const displayLessonBtn = (lessonBtns) => {
  for (const lessonBtn of lessonBtns) {
    ...
    newlessonBtn.innerHTML = ` <button id= "btn-${lessonBtn.level_no}" onclick ="loadGridData(${lessonBtn.level_no})"
          class="btn text-indigo-600 border-indigo-600 hover:bg-indigo-400 hover:text-white"
        >
...
const displayDetails = (detailsData) => {
 ...
 <div class="modal-box">
       ...
       <div id="synonym-buttons" class="flex gap-2 flex-wrap">  </div>
        <div class="modal-action justify-start">
          <form method="dialog">
            ...
            <button class="btn bg-indigo-600 text-white font-light">
              Complete Learning
            </button>
          </form>
        </div>
  ...
  if (!detailsData.synonyms || detailsData.synonyms.length === 0) {
    synonymButtons.innerHTML = `<button class="btn">No similar word found</button>`;
  } else {
